...
class DefaultPagePartGenerator extends KunstmaanGenerator
{
    ...
    private function generatePagePartEntity()
    {
        ...
        // Add some extra parameters in the generated entity because one could need them in there.
        $params = array(
        ...
        $entityCode = $this->render('Entity/PageParts/'.$this->entity.'/generatorTemplate.php', $params);
        $extraCode = $this->render('Entity/PageParts/ExtraFunctions.php', $params);

        // Take EntityCode and ExtraCode and combine them
    ...
    {
        // Add some extra parameters in the generated FormType because one could need them in there.
        $params = array(
            'bundle' => $this->bundle->getName(),
            'namespace' => $this->bundle->getNamespace(),
            'pagepart' => $this->entity,
        ...
        $formCode = $this->render('Form/PageParts/'.$this->entity.'/generatorTemplate.php', $params);
        $extraCode = $this->render('Form/PageParts/ExtraFunctions.php', $params);

        // Take FormCode and ExtraCode and combine them
        $pos = strrpos($formCode, "}");
        $trimmed = substr($formCode, 0, $pos);
        $formCode = $trimmed."\n".$extraCode."\n}";
        $formPath  = $this->bundle->getPath() . '/Form/PageParts/' . $this->entity . '.php';

        // Write class to filesystem
        $this->filesystem->mkdir(dirname($formPath));
    ...
    {
        $ResourcePath = $this->bundle->getPath().'/Resources/views/PageParts/'.$this->entity.'/view.html.twig';
